...
class Algorithm:
    ...
    def search(self, algorithm="kmeans"):
        ...
        if len(values) > 0:
            ...
            if 'spectral' in ls_sel_algorithms:
                list(ls_sel_algorithms).remove('spectral')

            if 'birch' in ls_sel_algorithms:
                list(ls_sel_algorithms).remove('birch')
            
            if len(ls_sel_algorithms) > 1 :
                return random.choice(ls_sel_algorithms)
            elif len(ls_sel_algorithms) == 1 :
                return ls_sel_algorithms[0]
            else:
                random.choice(['kmeans', 'ag', 'meanshift'])
        else:
            ls_rand_algorithms = ['kmeans', 'ag', 'meanshift']
