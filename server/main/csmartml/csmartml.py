...
from csmartml.HyperPartitions import HyperPartitions
...
class CSmartML:
	def __init__(self, filename, population, time_budget, publish, meta_cvi=False, algorithm=None, cvi=None, dataset=None, result="multi"):
		...
		if meta_cvi:
			...
			self.cvi = MetaCVI(filename, "distance", dataset).search(self.algorithm)
...
def ea_custom(pid, t, population, toolbox, cxpb, mutpb, publish, res):
	...
	while time.time() - start_time <= t:
		...
		# Replace the current population by the offspring
		# population[:] = offspring
		population[:] = toolbox.select(select_from, 10)  # Mu plus lambda : ea
